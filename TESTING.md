...
## v1.10.36 运行稳定性与前台轮询优化

- [x] 修改前归档 v1.10.35 到 `旧版本归档\2026-07-14_2330_v1.10.35_代码优化前`。
- [x] 静态检查确认托盘恢复按 1000/3000/8000/15000/30000ms 退避，达到 30 秒后持续重试，不再进入永久耗尽状态。
- [x] 静态检查确认托盘窗口句柄通过原子变量跨线程发布，重新添加图标前构造完整 `NOTIFYICONDATAW`。
- [x] QA 同时启动两个实例：第二个实例在 5 秒内以代码 0 退出，第一个实例继续运行并通过现有退出命令干净结束。
- [x] 静态检查确认每轮前台识别只构造一个快照，游戏目标选择和桌面显示判定复用进程名、路径及全屏结果。
- [x] 静态检查确认诊断导出的显存路径与其他 LHWM 路径在同一锁内复制。
- [x] 静态检查确认配置保存不再占用 LHWM 状态锁，临时文件验证失败或原子替换失败时保留脏状态。
- [x] QA 正常退出后没有新增 `Atomic config save failed` 或 `Temporary config validation failed` 日志。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 无警告通过。
- [x] `FPSOVERLAY_UI_QA` x64 编译无警告通过。
- [x] Windows `FileVersion` 为 `1.10.36.0`，`ProductVersion` 为 `v1.10.36 (2026-07-14 23:35)`，Release/QA manifest 均为 `1.10.36.0`。
- [x] 源码差异未触及游戏加加主绘制块、七个设置页面编号、指标顺序或绘制样式。
- [x] 2026-07-15 实机休眠唤醒后，自启动机制启动 v1.10.36，日志确认 LHWM 初始化成功。
- [x] 唤醒后进入《鸣潮》测试，日志确认游戏目标、Overlay 宿主和 FPS 数据恢复正常。
- [x] 已区分旧版本 v1.10.35 在 2026-07-14 23:02 的托盘重试耗尽记录，不将其计入 v1.10.36 结果。
- [ ] 用户实机休眠/唤醒后确认托盘最终恢复，日志不再出现 `Tray icon restore retries exhausted`。
- [ ] 在休眠前先运行同一份 v1.10.36 正式程序，再验证原进程是否保持存活以及托盘图标是否由持续重试恢复。
- [ ] 用户实机重启 Explorer 或等待 Shell 暂时无响应后，确认托盘可在后续退避周期自动恢复。
- [ ] 用户实机重复启动正式程序，确认只保留一个进程且已有实例打开设置。
- [ ] 用户实机测试游戏与普通最大化应用，确认 FPS 目标和“仅桌面显示”行为与 v1.10.35 一致。

## v1.10.37 WMI 与频率异步采集

- [x] 修改前归档 v1.10.36 到 `旧版本归档\2026-07-15_1906_v1.10.36_WMI与频率异步修复前`。
- [x] 静态检查确认主线程不再直接调用 `QueryCpuTemperature()`，WMI 工作线程在 WMI 释放前停止并等待退出。
- [x] WMI 枚举的 `Next` 等待上限为 500ms，返回无效值时不覆盖有效 LHWM CPU 温度。
- [x] 静态检查确认原 `PollClockSensors()` 主线程路径已移除，CPU/GPU LHWM 频率读取已并入异步硬件快照。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 通过。
- [x] `FPSOVERLAY_UI_QA` x64 编译通过，生成 `overlay_uiqa_v1.10.37.exe`。
- [x] Release/QA manifest 同步为 `1.10.37.0`，版本时间为 `v1.10.37 (2026-07-15 19:11)`。
- [x] 源码差异未触及游戏加加主绘制块、七个设置页面、指标顺序或绘制样式。
- [x] 本次未启动正式 `overlay.exe`。
- [ ] 用户实机验证 WMI 温度提供者迟缓或不可用时，Overlay 和设置窗口仍可响应。
- [ ] 用户实机确认 CPU/GPU 频率保持刷新，长时间游戏不出现因频率读取导致的周期性卡顿。
- [ ] 用户实机休眠/唤醒后确认 WMI 温度回退和 LHWM 频率快照能继续恢复。

## v1.10.38 游戏结束性能报告

- [x] 修改前归档 v1.10.37 到 `旧版本归档\2026-07-16_1439_v1.10.37_游戏性能报告前`，正式程序和源码包哈希与归档前一致。
- [x] QA 生命周期自检确认：目标稳定 2 秒后开始、短暂丢失不结束、PID 切换完成旧局并开始新候选、上一局报告在新局开始后仍保留。
- [x] QA 生命周期自检确认：关闭功能立即丢弃 `activeSession`，但不清空上一份已完成报告。
- [x] QA 阈值自检确认：短局不发出自动打开请求，有效样本时间跨度达到 30 秒时只发出一次可消费请求。
- [x] QA 数学自检使用 200 个 FPS 样本验证平均、最低、最高和最低 1% 平均值；1% Low 结果为 `1.5`，不是单个最低 FPS。
- [x] QA 数学自检确认无效温度样本不按 0 计入统计；两个有效温度 `50/70°C` 的平均、最低、最高和样本数分别为 `60/50/70/2`。
- [x] QA 数学自检确认整机功耗按梯形法积分，估算样本参与后报告标记为估算。
- [x] QA 后台持久化自检在中文路径写入 CSV，确认 UTF-8 BOM、安全临时文件提交和线程退出刷新后删除测试文件。
- [x] 静态检查确认报告 Feature 位于 `LaptopPowerFeature` 之后，且没有主动调用 LHWM、WMI、ETW 或 PDH。
- [x] 静态检查确认 28,800 样本上限、功耗覆盖率提示、N/A 空状态和退出保存路径均存在。
- [x] 关闭自动打开后，完成会话仍会转移到 `lastCompletedSession`，可从“游戏报告”页面查看。
- [x] 源码差异未触及游戏加加 `drawSeg()`、Steam、垂直或水平 Overlay 的指标顺序和绘制样式。
- [x] `FPSOVERLAY_UI_QA` 以 `/W4 /permissive- /Zi` 无警告编译通过，自检进程持续运行并响应后通过 UI 线程正常退出。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
- [x] Windows `FileVersion` 为 `1.10.38.0`，`ProductVersion` 为 `v1.10.38 (2026-07-16 14:51)`。
- [x] 本次未启动正式发布目录中的 `overlay.exe`。
- [ ] 用户实机启动游戏并运行超过 30 秒后退出，确认自动打开完整报告并生成 `GameSessionReports` CSV。
- [ ] 用户实机短暂切换桌面超过 1 秒但少于 5 秒，确认不会提前结束会话。
- [ ] 用户实机从一个游戏切换到另一个游戏，确认旧局完成后新局重新稳定 2 秒开始。
- [ ] 用户实机让温度或功耗传感器暂时失效，确认页面和 CSV 显示 N/A 而不是 0。
- [ ] 用户实机关闭自动打开，确认游戏结束后不弹窗但“游戏报告”页面仍保留最近一局。
- [ ] 用户实机确认真实整机传感器显示“实际用电量”，G815LR 融合模型显示“估算用电量”和覆盖率。
- [ ] 用户实机在游戏进行中退出 FPS Overlay，确认退出清理阶段保存当前会话且不遗留 `.tmp` 文件。
- [ ] 用户实机进行长时间游戏，对比启用/关闭报告功能的 Overlay FPS 和帧时间，确认每秒缓存采样无可感知影响。

## v1.10.39 游戏报告实机问题修复

- [x] 修改前归档 v1.10.38 到 `旧版本归档\2026-07-16_1648_v1.10.38_实机报告修复前`，归档包含三份用户实测 CSV。
- [x] 检查三份 CSV 均带 UTF-8 BOM，真实《鸣潮》会话共 4,293 条逐秒样本，CSV 写入和退出刷新正常。
- [x] 定位两份 `explorer.exe` 假会话；报告目标现在拒绝已知桌面进程、系统 Shell 和 FPSOverlay 自身。
- [x] 定位真实会话仅前 4 条 CPU 温度/功耗、GPU 占用/温度/功耗和显存有效：假报告自动打开设置中心后，旧逻辑暂停了异步硬件轮询。
- [x] 游戏报告处于候选或记录状态时保持原异步硬件轮询；没有在报告 Feature 内增加 LHWM、WMI、ETW 或 PDH 查询。
- [x] 估算整机功耗遇到过期硬件快照时按 N/A 处理，不计入梯形能耗积分或覆盖率。
- [x] 定位游戏内监控窗消失原因：假会话在真实游戏 PID 接管时自动打开报告，使同一窗口切换到设置中心。
- [x] PID 直接切换到另一游戏时不再自动打开旧报告，旧报告仍保留并按配置保存。
- [x] QA 自检覆盖：桌面目标排除、过期估算功耗排除、直接游戏切换不弹出旧报告。
- [x] `FPSOVERLAY_UI_QA` 以 `/W4 /permissive- /Zi` 无警告编译通过；运行 5 秒保持响应，自检通过并以代码 0 正常退出。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
- [x] Windows `FileVersion` 为 `1.10.39.0`，`ProductVersion` 为 `v1.10.39 (2026-07-16 16:51)`。
- [x] 源码差异未触及游戏加加 `drawSeg()`、Steam、垂直或水平 Overlay 的指标顺序和绘制样式。
- [x] 本次未启动 v1.10.39 正式发布程序。
- [ ] 用户复测桌面静置和打开 Steam，确认不再生成 `explorer.exe` 游戏报告。
- [ ] 用户保持设置中心打开后进入游戏，确认 Overlay 仍显示且 GPU/温度/功耗持续采样。
- [ ] 用户从一个游戏直接切换到另一个游戏，确认旧报告不会抢占新游戏 Overlay。
- [ ] 用户退出游戏且没有后续游戏目标，确认 5 秒后报告正常自动打开。

## v1.10.40 反作弊引导进程误识别修复

- [x] 修改前归档 v1.10.39 到 `旧版本归档\2026-07-16_1757_v1.10.39_GameGuard误识别修复前`，归档保留五份实测 CSV。
- [x] v1.10.39 实测没有新增 `explorer.exe` 报告，证明桌面进程排除生效。
- [x] 《绝地潜兵 2》报告包含 1,556 条样本；CPU/GPU/显存/整机功耗均有 1,555 条有效数据，仅一秒因缓存过期显示 N/A。
- [x] 功耗覆盖率为 99.9%，平均整机估算功耗 211.81W，25 分 59 秒估算用电量 0.091584kWh，积分数量级一致。
- [x] 游戏开始时没有旧报告自动打开记录；游戏目标清空后报告正常打开，未再次出现报告抢占新游戏 Overlay 的日志证据。
- [x] 定位 `GameGuard.des` 被误记为 6 秒、5 个有效样本的假会话。
- [x] GameGuard、GameMon、Easy Anti-Cheat、BattlEye 和受保护游戏引导程序已加入现有非游戏进程排除表。
- [x] QA 名称分类自检确认：带描述的 `GameGuard.des (...)` 和 EAC 被排除，`helldivers2.exe (...)` 保持可识别。
- [x] `FPSOVERLAY_UI_QA` 以 `/W4 /permissive- /Zi` 无警告编译通过；运行 5 秒保持响应，自检通过并以代码 0 正常退出。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
...
- [x] 修改前已归档 v1.10.40 到 `旧版本归档\2026-07-16_1818_v1.10.40_历史记录功能前`，归档保留五份实测 CSV。
- [x] 历史索引最多保留最新 100 份 CSV，列表按修改时间降序展示游戏、进程、开始时间、时长和平均 FPS。
- [x] 静态检查确认目录扫描、CSV 摘要和完整会话解析只在后台线程执行；每帧页面只复制内存快照。
- [x] QA 持久化自检覆盖 UTF-8 BOM 写入、中文游戏/进程摘要读取、完整逐秒样本回读和线程退出刷新。
- [x] CSV 回读为每个指标独立恢复有效标记，N/A 不按 0 进入统计。
- [x] 新会话完成或用户切换选择时会递增加载序号，过期的历史加载结果不会覆盖当前报告。
- [x] `FPSOVERLAY_UI_QA` 以 `/W4 /permissive- /Zi` 无警告编译通过；运行 5 秒保持响应，自检通过并以代码 0 正常退出。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
- [x] Windows 版本已升级为 `v1.10.41 (2026-07-16 18:29)`。
- [x] 本次未启动 v1.10.41 正式程序，未修改游戏加加、Steam、垂直或水平 Overlay 绘制逻辑。
- [ ] 用户在“游戏报告”页确认既有五份 CSV 按时间降序显示。
- [ ] 用户点击不同历史记录，确认摘要卡、硬件信息和十种曲线切换到对应会话。
- [ ] 用户完成新游戏会话后，确认本次报告优先显示，且新 CSV 自动加入历史列表。

## v1.10.42 设置中心与报告查看界面升级

- [x] 修改前已归档 v1.10.41 到 `旧版本归档\2026-07-16_1854_v1.10.41_整体界面升级前`。
- [x] 设置中心使用独立 Segoe UI Variable + 微软雅黑字体，默认 Overlay 字体保持原实现。
- [x] Windows 配置窗口深色标题栏、系统圆角、编号侧栏、页头、状态卡和通用功能卡均在 `FPSOVERLAY_UI_QA` 中实际渲染检查。
- [x] 使用五份真实历史 CSV 检查报告页：设置、历史选择、会话摘要和五项关键指标可同时进入首屏。
- [x] 历史选中行、记录总数、双栏会话/能耗摘要、紧凑覆盖率和文件路径悬停提示均正常显示。
- [x] 游戏报告空状态、后台读取状态和 N/A 逻辑未修改；没有新增每帧计算、传感器查询或文件 IO。
- [x] `SettingsUi` 样式颜色/变量及独立设置字体的 Push/Pop 栈保持配对。
- [x] 源码差异未触及游戏加加 `drawSeg()`、Steam、垂直或水平 Overlay 的指标顺序和绘制样式。
- [x] `FPSOVERLAY_UI_QA` v1.10.42 以 `/W4 /permissive- /Zi` 无警告编译通过，连续运行 5 秒保持响应并正常关闭。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
- [x] Windows `FileVersion` 为 `1.10.42.0`，`ProductVersion` 为 `v1.10.42 (2026-07-16 19:05)`。
- [x] 本次未启动 v1.10.42 正式发布程序。
- [ ] 用户在 100%、125% 和 150% Windows 缩放下检查八个设置页面，无文字裁切或控件重叠。
- [ ] 用户使用鼠标滚轮浏览长报告，确认曲线与硬件快照滚动顺畅。

## v1.10.43 游戏报告界面密度修正

- [x] 修改前已归档 v1.10.42 到 `旧版本归档\2026-07-16_1922_v1.10.42_报告界面密度修复前`。
- [x] 使用用户提供的 150% 高 DPI 实机照片定位历史选中态、摘要留白、覆盖率长条和指标卡裁切问题。
- [x] 使用相同五份真实历史 CSV 在 `FPSOVERLAY_UI_QA` 中复现并复查报告页。
- [x] 历史当前行已改为低饱和背景、左侧标记和“当前”文字，操作列不再显示被挤压的禁用按钮。
- [x] 会话摘要改为响应式键值表，功耗覆盖率长条已移除，记录文件与操作保持同一行。
- [x] 摘要标签与“打开文件夹”操作列按当前字体计算宽度，高 DPI 下不再依赖固定像素宽度。
- [x] FPS、CPU、GPU、内存和整机功耗卡片均在两行内显示；CPU/GPU 温度、1% Low 和功耗来源无裁切。
- [x] 源码差异仅位于游戏报告绘制和版本/文档，未修改游戏加加、Steam、垂直或水平 Overlay 绘制逻辑。
- [x] `FPSOVERLAY_UI_QA` v1.10.43 以 `/W4 /permissive- /Zi` 无警告编译通过，并完成真实 CSV 页面截图检查。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
- [x] Windows `FileVersion` 为 `1.10.43.0`，`ProductVersion` 为 `v1.10.43 (2026-07-16 19:26)`。
- [x] 本次未启动 v1.10.43 正式发布程序。
- [ ] 用户在正式 v1.10.43 中再次打开同一报告，确认实际屏幕观感与 QA 截图一致。

## v1.10.44 游戏报告高 DPI 遮挡修正

- [x] 修改前已归档 v1.10.43 到 `旧版本归档\2026-07-16_1950_v1.10.43_报告遮挡修复前`。
- [x] 确认历史表旧高度固定为 `174px`，在 150% 高 DPI 下只能完整显示约 2 条双行记录。
- [x] 历史表改为字体高度驱动，`FPSOVERLAY_UI_QA` 使用 5 份真实 CSV 确认首屏完整显示 4 条记录。
- [x] 开始时间、时长、平均 FPS 和操作列在高 DPI 下完整显示，无列文字重叠或按钮裁切。
- [x] 滚动到报告下半部确认：文件名与打开按钮无遮挡，五张指标卡按 4+1 排列且文字完整，曲线与硬件卡无覆盖。
- [x] 窄宽度逻辑使用实际文本宽度决定摘要列数、文件行布局、指标卡列数及卡片高度。
- [x] `FPSOVERLAY_UI_QA` v1.10.44 以 `/W4 /permissive- /Zi` 无警告编译通过，并完成真实 CSV 视觉检查。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
- [x] Windows `FileVersion` 为 `1.10.44.0`，`ProductVersion` 为 `v1.10.44 (2026-07-16 19:58)`。
- [x] 本次未启动 v1.10.44 正式发布程序。
- [x] 用户退出旧版管理员进程后，发布目录根部 `overlay.exe` 已同步并核对为 v1.10.44。

## v1.10.45 游戏加加风格设置中心升级

- [x] 修改前已归档 v1.10.44 到 `旧版本归档\2026-07-16_2019_v1.10.44_GamePP界面升级前`。
- [x] 参考游戏加加官网、官方硬件监控教程、官方性能报告页面、官方公开视频目录和公开程序截图，提炼窄侧栏、石墨底色、扁平分组、青色交互、琥珀关键数值和高信息密度规则。
- [x] `FPSOVERLAY_UI_QA` 逐页检查控制中心、监控项目、游戏内监控、桌面显示、控制与启动、硬件信息、提醒与功耗和游戏报告。
- [x] 使用发布目录中的 5 份真实 CSV 检查报告首屏、历史表、会话摘要、五项指标卡、完整曲线和硬件快照。
- [x] 历史表完整显示表头加 4 条双行记录；`1 小时 11 分钟 34 秒` 与平均 FPS 列之间无重叠。
- [x] FPS 阈值和电池辅助供电滑块改为上方标签，满宽控件右侧不再绘制被裁切的外部标签。
- [x] 报告五张指标卡在当前 QA 宽窗口中同排显示，1% Low、CPU/GPU 平均温度和功耗来源均在独立行完整显示。
- [x] 源码差异未触及 `drawSeg()`、Steam、垂直或水平 Overlay 绘制路径。
- [x] `FPSOVERLAY_UI_QA` v1.10.45 以 `/W4 /permissive- /Zi` 无警告编译通过，并完成多页面与滚动区视觉复核。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
- [x] Windows `FileVersion` 为 `1.10.45.0`，`ProductVersion` 为 `v1.10.45 (2026-07-16 20:35)`。
- [x] 本次只运行 `FPSOVERLAY_UI_QA`；Codex 未启动 v1.10.45 正式发布程序。

## v1.10.46 耗电量单位改为度

- [x] 修改前已归档 v1.10.45 到 `旧版本归档\2026-07-16_2215_v1.10.45_耗电量单位修改前`。
- [x] 复核最新实测 `20260716-214425_CrimsonDesert.exe.csv`：会话 28 分 40 秒，功耗覆盖率 99.7%，估算耗电量 0.103370 kWh，改单位后应为 0.103370 度。
- [x] QA 持久化自检确认新 CSV 包含“实际用电量(度)”，且不再写入“用电量(kWh)”。
- [x] `FPSOVERLAY_UI_QA` v1.10.46 以 `/W4 /permissive- /Zi` 无警告编译通过，使用最新实测报告确认页面显示 `0.103370 度`。
- [x] Release x64 资源编译、C++ 编译、链接和 LTCG 以 `/W4 /permissive- /Zi` 无警告通过。
- [x] Windows `FileVersion` 为 `1.10.46.0`，`ProductVersion` 为 `v1.10.46 (2026-07-16 22:15)`。
