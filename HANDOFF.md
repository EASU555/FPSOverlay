...
- 当前发布文件：
  ...
  - `overlay_v1.10.39_20260716-1651.exe`
  - `overlay_v1.10.40_20260716-1758.exe`
  - `overlay_v1.10.41_20260716-1829.exe`
  - `overlay_v1.10.42_20260716-1905.exe`
  - `overlay_v1.10.43_20260716-1926.exe`
  - `overlay_v1.10.44_20260716-1958.exe`
  - `overlay_v1.10.45_20260716-2035.exe`
  - `overlay_v1.10.46_20260716-2215.exe`
  - `overlay_v1.10.47_20260716-2325.exe`
  - `overlay_v1.10.48_20260717-1341.exe`
  - `overlay_v1.10.49_20260722-1857.exe`
- v1.10.49 发布程序 SHA256：`423C644992E3329528D5DEEC1E6742B5E256A4F0899CFA842EFAC22DDB51368E`
- v1.10.49 Windows ZIP SHA256：`AC6B6F64D90822CBEA9510A64FC53FE499FEFD06713ABBADBF9AEB6BEE6A6EB4`
- 发布目录根部 `overlay.exe` 已同步为 `v1.10.49`，SHA256 同上。
- 最近归档：`旧版本归档\2026-07-22_1856_v1.10.48_shadPS4无边框Overlay修复前`
...
`v1.10.49` 继承现有 UI 与显示逻辑，并保留 v1.10.18 的 G815LR 插座校准：

- 中性石墨灰底色，青蓝、绿色和琥珀色作为交互、正常与注意状态强调色。
- 窄单行编号侧边导航、FPSOverlay 自有品牌文字和一体化首页状态栏。
...
- “游戏报告”页展示最新 100 份本地 CSV 历史摘要，可切换查看任一场的完整统计和曲线。
- 设置中心使用独立 Segoe UI Variable + 微软雅黑字体、系统深色标题栏和 4～8px 克制圆角；游戏内 Overlay 字体与绘制保持原样。
- 八个页面统一中性石墨表面、页签式页头、扁平分组和紧凑控件，青色用于主操作，绿/橙用于正常与注意状态。
- 游戏报告首屏采用横向设置、紧凑历史表、双栏会话/能耗摘要与五项关键指标，完整曲线位于硬件快照之前。
- v1.10.43 将报告摘要收敛为响应式键值表，历史当前行使用低饱和标记，指标卡补充数据保持完整可见。
- 历史目录扫描与 CSV 解析使用独立后台线程，每帧绘制只读内存快照；新报告保存成功后自动刷新。
...
- `v1.10.49` 根据 2026-07-22 多次 shadPS4 实测日志修复游戏内不显示 Overlay：ETW/FPS/报告正常，故障是 shadPS4 强游戏身份缺失与 Vulkan 无边框顶层 HWND 切换没有触发宿主重置。
- `shadPS4.exe` 已纳入强游戏识别；Overlay 宿主现在比较显示器矩形、目标 PID 和 HWND，新游戏窗口会执行一次不抢焦点的置顶切换。
- v1.10.49 Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过；QA 运行 4 秒保持响应并以代码 0 退出。
- v1.10.49 Windows `FileVersion` 为 `1.10.49.0`，`ProductVersion` 为 `v1.10.49 (2026-07-22 18:57)`；正式 Release 未启动。
- `v1.10.48` 根据真实日志修复 `ACE-Helper.exe` 被最近目标保持误认并生成 5 秒无 FPS 报告的问题；已生成的历史 CSV 保留不删。
- 用户实测 v1.10.47 从 2026-07-17 00:15 休眠至 13:04 后仍由同一进程继续运行；托盘在 Shell 返回错误 1008 后自动重试恢复，LHWM 15 秒延迟恢复并应用新鲜硬件快照。
- 最新真实游戏报告 `SilverStarGameWeGame.exe` 时长 20 分 57 秒，共 1254 个逐秒样本，功耗覆盖率 100.0%，估算耗电量 0.068968 度。
- v1.10.48 Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过；QA 运行 4 秒保持响应并以代码 0 退出。
- v1.10.48 Windows `FileVersion` 为 `1.10.48.0`，`ProductVersion` 为 `v1.10.48 (2026-07-17 13:41)`；Release 输出为 `overlay_v1.10.48_20260717-1341.exe`。
- `v1.10.47` 完成对抗性审查修复：PawnIO 提权临时文件、未知高 FPS 假游戏、恶意/过大历史 CSV、超长会话统计、电池 IOCTL 退出延迟、CSV 公式注入、配置完整性、报告覆盖和 Unicode 路径均已加固。
- v1.10.47 长会话曲线使用有界降采样，但统计、功耗覆盖率和用电量基于全量逐秒样本；“1% Low”明确标注为逐秒样本口径。
- v1.10.47 Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过；最终 QA 运行 4 秒保持响应，内置自检通过并以代码 0 退出。
- v1.10.47 Windows `FileVersion` 为 `1.10.47.0`，`ProductVersion` 为 `v1.10.47 (2026-07-16 23:25)`，Release/QA manifest 同步为 `1.10.47.0`。
- v1.10.47 Release 输出为 `overlay_v1.10.47_20260716-2325.exe`，发布目录根部 `overlay.exe` 与版本化 EXE 均已同步。
- `v1.10.46` 将游戏报告页和新 CSV 的耗电量单位由 `kWh` 改为“度”，内部 Wh 梯形积分与数值保持不变。
- 最新实测 CrimsonDesert 报告在 QA 页面显示 `0.103370 度`；旧 `(kWh)` CSV 无需迁移即可继续查看。
- v1.10.46 Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过，Release SHA256 为 `F7A6AC2AD9659E313C7CBDE888F1EE1A4E8D06C905FD13832CF9C04D1EB3F1B3`。
- 发布目录已生成 v1.10.46 版本化 EXE 和 Windows ZIP；根部 `overlay.exe` 因用户正在运行 v1.10.44 而保持旧版，未强制结束进程。
- `v1.10.45` 参考游戏加加公开界面，将设置中心升级为窄侧栏、高密度石墨监控控制台；没有复制品牌素材或修改 Overlay。
- v1.10.45 使用 5 份真实 CSV 复核八个页面、历史 4 行、长时长列、五项摘要卡、曲线和硬件快照，未发现文字或控件遮挡。
- v1.10.45 修正 FPS 阈值、监控 GPU 和电池辅助供电满宽控件的标签裁切；报告长会话时长不再侵入平均 FPS 列。
- v1.10.45 Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过，Release SHA256 为 `5EDD32B88CD1F92092884D41010441D4EE8A4293FE38DD375A061E8B56340BF9`。
- v1.10.44 已归档到 `旧版本归档\2026-07-16_2019_v1.10.44_GamePP界面升级前`。
- `v1.10.44` 将历史表高度改为字体驱动，在 150% 高 DPI 下完整显示 4 条双行历史记录。
- v1.10.44 的历史列宽、摘要布局、文件行、指标卡列数和卡片高度均按实际字体尺寸计算。
- `v1.10.44` 使用 5 份真实 CSV 完成 QA 首屏与下半部滚动检查，文字、按钮、指标卡、曲线和硬件信息无遮挡。
- v1.10.44 Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过，Release SHA256 为 `B776094B335C04A7F7522CD7B08F2A88064275943E6F684325714687FEDFBF76`。
- 用户退出旧版管理员进程后，发布目录根部 `overlay.exe` 已同步并核对为 v1.10.44。
- v1.10.43 已归档到 `旧版本归档\2026-07-16_1950_v1.10.43_报告遮挡修复前`。
- `v1.10.43` 根据用户 150% 高 DPI 实机照片修复报告页过度拉伸、整行亮蓝选中、覆盖率长条和指标说明裁切。
- v1.10.43 使用同一组五份真实历史 CSV 完成 QA 复查；摘要、当前行、文件操作和五张指标卡均完整显示。
- `v1.10.43` Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过。
- v1.10.43 Release 输出为 `overlay_v1.10.43_20260716-1926.exe`，SHA256 为 `A0A875B952EC36DA677FD6967C8D69FEBD11FAF15381C7EC1DD74D3AAA414CBD`。
- v1.10.42 已归档到 `旧版本归档\2026-07-16_1922_v1.10.42_报告界面密度修复前`。
- `v1.10.42` 完成设置中心和游戏报告查看界面升级；正式逻辑、会话采样、CSV 和 Overlay 绘制未修改。
- v1.10.42 使用五份真实历史 CSV 完成 QA 视觉检查；深色标题栏、八页导航、历史选中态、报告摘要与指标卡均正常显示。
- `v1.10.42` Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过；QA 连续运行 5 秒保持响应。
- v1.10.42 Release 输出为 `overlay_v1.10.42_20260716-1905.exe`，SHA256 为 `90E5C868A9C2B5CD640DC5524BD2A04258A88879893929EF436F5EBE1638636C`。
- v1.10.41 已归档到 `旧版本归档\2026-07-16_1854_v1.10.41_整体界面升级前`，归档包含当时的源码、发布目录和源码包。
- `v1.10.41` 在现有“游戏报告”页新增最新 100 份 CSV 历史列表，支持后台读取并复用现有统计卡和十种曲线。
- v1.10.41 目录扫描、摘要读取和完整 CSV 解析均位于可停止后台线程；加载序号防止过期结果覆盖新会话。
- `v1.10.41` Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过；QA 运行 5 秒保持响应，自检通过并以代码 0 正常退出。
- v1.10.41 Release 输出为 `overlay_v1.10.41_20260716-1829.exe`，SHA256 为 `0C4A5D76AAE481C429526C9E53160A9922CE07E2D3DC552EBA99472918B2553D`。
- v1.10.40 已归档到 `旧版本归档\2026-07-16_1818_v1.10.40_历史记录功能前`，其中保留五份用户实测 CSV。
- `v1.10.40` Release 和 `FPSOVERLAY_UI_QA` 均以 `/W4 /permissive- /Zi` 无警告编译通过；QA 名称分类及既有报告自检运行通过并以代码 0 正常退出。
- v1.10.40 Release 输出为 `overlay_v1.10.40_20260716-1758.exe`；用户正常退出 v1.10.39 后，发布目录根部 `overlay.exe` 已同步并核对为 v1.10.40。
