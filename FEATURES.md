...
- 当前版本为 `v1.10.49 (2026-07-22 18:57)`。
- 当前功能线每交付一次更新递增修订号。
...
## shadPS4 无边框全屏显示修复（v1.10.49）

- 根据 2026-07-22 实测日志，`shadPS4.exe` 的 ETW FPS 和游戏报告正常，但启动阶段会先被当作普通程序隐藏 Overlay。
- `shadPS4.exe` 现纳入强游戏身份，无需等待未知进程的持续 ETW 兜底时间即可锁定游戏目标。
- Overlay 宿主变化检测新增目标 PID 和顶层 HWND；Vulkan 无边框窗口替换但显示器矩形不变时，仍会触发一次不激活的置顶切换和 DWM 重组。
- `SetWindowPos` 失败现会记录 PID、HWND 和 Windows 错误码，便于区分窗口层级问题与游戏识别问题。
- 保持无 owner 的外部透明 Win32 Overlay，不注入模拟器，不修改游戏加加、Steam、垂直或水平布局。
